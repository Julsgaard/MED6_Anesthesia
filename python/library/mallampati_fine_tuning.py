...
def run_mallampati_model():
    ...
    optimizer = optim.Adam(model.fc.parameters(), learning_rate)  # Only train the classifier parameters
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Epoch {epoch + 1}, Training Loss: {total_train_loss / len(train_loader)}")  # Print average training loss
        ...
        with torch.no_grad():
            ...
            print(
                f"Epoch {epoch + 1}, Validation Loss: {total_val_loss / len(test_loader)}")  # Print average validation loss
    ...
    # Save the model
    torch.save(model.state_dict(), f'mallampati_models/best_model_{best_acc * 100}%_{num_epochs}_epochs.pth')
